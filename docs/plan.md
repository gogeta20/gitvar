...
- Se agrego un componente `Tooltip` reutilizable en `core/components/`, con estilo propio de la app, para reemplazar el `title` nativo del navegador en el badge de refs.
...
- El panel de detalle de commit se conecto a datos reales de archivos cambiados (backend nuevo: `/api/commit-files` y `changedFiles` en `/api/status`), gano un panel de diff por archivo (`/api/file-diff`, dividido en hunks con numero de linea) que reemplaza el grafo central al abrir un archivo, un arbol de archivos anidado real con modos Tree/Path, letra y color de estado por archivo, ordenamiento y buscador. El workspace tambien paso a ser un "app shell": la pagina no scrollea, cada columna scrollea de forma independiente.
- Decision de persistencia de preferencias de UI: se usa `localStorage` via un hook generico (`usePersistedState`), igual que ya hacia `useResizableWidth` para el ancho del panel. Esto sigue siendo valido cuando la app se empaquete como desktop con Tauri, ya que el webview mantiene su propio `localStorage` persistente por app; no hace falta backend/SQLite para preferencias de UI livianas (colapsos, toggles, orden). SQLite se reserva para datos de dominio reales.
